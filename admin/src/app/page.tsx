...
import { tours, categories, siteSettings, blogPosts } from '@/lib/schema';
...
export default async function DashboardPage() {
  ...
  const [blogPublishedCount] = await db.select({ count: sql<number>`count(*)` }).from(blogPosts).where(sql`published = true`);
...
function Stat({ icon, label, value, color }: { icon: string; label: string; value: number; color: string }) {
  const colors: Record<string, string> = {
    ...
    emerald: 'bg-emerald-50 border-emerald-200', teal: 'bg-teal-50 border-teal-200',
  };
  return (
