...
export const metadata: Metadata = {
  title: 'Tin tức du lịch Trung Quốc | Lịch khởi hành, review, kinh nghiệm',
  description:
    'Tin tức Sơn Hằng Travel tổng hợp bài viết mới về Đông Hưng, Hà Khẩu, Nam Ninh, Vân Nam cùng lịch khởi hành, ưu đãi và kinh nghiệm đi tour thực tế.',
  alternates: {
    canonical: `${SITE_URL}/tintuc`,
  },
  openGraph: {
    title: 'Tin tức du lịch Trung Quốc | Sơn Hằng Travel',
    description:
      'Tin tức Sơn Hằng Travel tổng hợp bài viết mới về Đông Hưng, Hà Khẩu, Nam Ninh, Vân Nam cùng lịch khởi hành, ưu đãi và kinh nghiệm đi tour thực tế.',
    url: `${SITE_URL}/tintuc`,
    type: 'website',
    images: [
      {
        ...
        width: 1200,
        height: 630,
        alt: 'Tin tức du lịch Trung Quốc | Sơn Hằng Travel',
      },
    ],
  },
  twitter: {
...
function buildNewsItemListSchema(posts: BlogPost[]) {
  return {
    '@context': 'https://schema.org',
    '@type': 'CollectionPage',
    '@id': `${SITE_URL}/tintuc#collection`,
    url: `${SITE_URL}/tintuc`,
    name: 'Tin tức Sơn Hằng Travel',
    hasPart: {
      '@type': 'ItemList',
      ...
      <div className="grid lg:grid-cols-[1.08fr_0.92fr]">
        ...
        <div className="flex flex-col justify-center p-4 sm:p-6 md:p-8">
          ...
          <h2 className="mt-3 text-lg font-bold leading-tight text-gray-900 sm:mt-4 sm:text-2xl md:text-4xl">
            <Link href={getPostHref(post.slug)} className="transition-colors hover:text-emerald-700">
              {post.title}
            </Link>
          </h2>
...
function NewsGridCard({ post }: { post: BlogPost }) {
  return (
    <article className="overflow-hidden rounded-[24px] border border-gray-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-lg">
      ...
      <div className="p-3 sm:p-4 md:p-6">
        ...
        <h3 className="mt-2 text-[13px] font-bold leading-tight text-gray-900 sm:text-base md:mt-4 md:text-xl">
          <Link href={getPostHref(post.slug)} className="line-clamp-3 transition-colors hover:text-emerald-700 md:line-clamp-2">
            {post.title}
          </Link>
        </h3>
...
export default async function NewsPage() {
  ...
  const newsCollectionSchema = posts.length > 0 ? buildNewsItemListSchema(posts) : null
  ...
  return (
    <main className="bg-white">
      ...
      <section className="border-b border-gray-100 bg-[linear-gradient(180deg,#ffffff_0%,#f6fbf8_100%)]">
        <div className="mx-auto max-w-6xl px-4 py-8 md:py-14">
          ...
          <h1 className="mt-4 text-2xl font-bold text-gray-900 sm:text-3xl md:mt-5 md:text-5xl">Tin Tức Du Lịch Trung Quốc</h1>
      ...
      <section className="mx-auto max-w-6xl px-4 py-6 md:py-12">
        {posts.length === 0 ? (
          <div className="rounded-[28px] border border-dashed border-emerald-200 bg-white p-8 text-center shadow-sm">
            ...
            <p className="mt-3 text-gray-600">Khi admin đăng bài mới, trang tin tức sẽ tự cập nhật tại đây.</p>
          </div>
        ) : (
          <>
            {featuredPost && (
            ...
            {remainingPosts.length > 0 && (
              <div className="mt-8 md:mt-10">
                <div className="mb-4 md:mb-5">
                  ...
                  <h2 className="mt-1.5 text-xl font-bold text-gray-900 sm:text-2xl md:mt-2 md:text-3xl">Danh sách tin tức</h2>
                </div>
