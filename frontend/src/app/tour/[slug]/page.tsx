import { getTourBySlug, getImageUrl, TourData, getSiteSettings, getTours, getCategories } from '@/lib/data'
...
export default async function TourDetailPage({ params }: PageProps) {
  ...
  try {
    const [tour, siteSettings, categoriesData] = await Promise.all([
      getTourBySlug(slug),
      getSiteSettings(),
      getCategories(),
    ])
    ...
    const siblingCategories = (categoriesData || [])
      .filter((item) => item.slug !== tour.categorySlug && (item.tourCount || 0) > 0)
      .slice(0, 4)
